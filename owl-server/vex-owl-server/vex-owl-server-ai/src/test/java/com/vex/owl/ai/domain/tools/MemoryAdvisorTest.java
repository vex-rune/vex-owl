...
class MemoryAdvisorTest extends SpringIntegrationTest {
    ...
    private String invokeBuildEnrichedPrompt(String tenantId, String sessionId, String prompt) {
        try {
            var method = MemoryAdvisor.class.getDeclaredMethod("buildEnrichedPrompt", String.class, String.class, String.class);
            method.setAccessible(true);
            return (String) method.invoke(memoryAdvisor, tenantId, sessionId, prompt);
    ...
    void buildEnrichedPrompt_shouldInjectUserMemoryAndHistory() {
        String tenantId = "advisor-tenant-1";
        String sessionId = "advisor-session-1";

        userMemoryService.addMemory(tenantId, "preference", "用户喜欢简洁回答", 90);
        userMemoryService.addMemory(tenantId, "fact", "用户是程序员", 80);

        chatManager.createSession(tenantId, "测试");
        chatManager.saveMessages(List.of(
                ChatMessageEntity.builder().tenantId(tenantId).conversationId(sessionId).messageType("USER").textContent("之前的问题").build(),
                ChatMessageEntity.builder().tenantId(tenantId).conversationId(sessionId).messageType("ASSISTANT").textContent("之前的回答").build()
        ));

        String result = invokeBuildEnrichedPrompt(tenantId, sessionId, "你好");
    ...
    void buildEnrichedPrompt_shouldLimitHistory() {
        String tenantId = "advisor-limit-tenant";
        String sessionId = "advisor-limit-session";

        chatManager.createSession(tenantId, "测试");
        for (int i = 0; i < 30; i++) {
            chatManager.saveMessages(List.of(
                    ChatMessageEntity.builder().tenantId(tenantId).conversationId(sessionId).messageType("USER").textContent("消息" + i).build()
            ));
        }

        String result = invokeBuildEnrichedPrompt(tenantId, sessionId, "新消息");
